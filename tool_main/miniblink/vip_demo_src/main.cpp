...
void initDllPath()
{
    ...
    if (::PathFileExistsW(&mbPath->at(0))) {
        ...
        ::PathAppendW(&mbMainPath->at(0), L"miniblink_4957_x32.dll");
        mbSetMbMainDllPath(&mbMainPath->at(0));
    } else {
        delete mbPath;
        mbPath = new std::vector<wchar_t>(path);

        ::PathAppendW(&mbPath->at(0), L"..\\..\\miniblink_4957_x32.dll");
        mbSetMbDllPath(&mbPath->at(0));
    ...
    _In_ int       nCmdShow)
{
    ...
    mbSetMbDllPath(L"G:\\mycode\\mbvip\\out\\x86\\Debug\\mb.dll");
